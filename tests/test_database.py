...
from metatube.database import Database, Templates
...
class TestDatabase(unittest.TestCase):
    ...
    def testAutoCreation(self):
        ...
        relative_url = "sqlite:///" + os.path.join(
            "metatube", Config.SQLALCHEMY_DATABASE_URI.replace("sqlite:///", "")
        )
    ...
    def testConfigTable(self):
        ...
        config.set_hw_transcoding("VAAPI")
    ...
    def testTemplatesTable(self):
        ...
        self.assertEqual(Templates.count_templates(), 1)
        ...
        newTemplate = Templates.fetch_template(newTemplateId)
        newTemplate.set_default(defaultTemplate)  # type: ignore
        self.assertEqual(newTemplateId, 1)
        self.assertEqual(
            Templates.fetch_all_templates(), [defaultTemplate, newTemplate]
        )
        self.assertEqual(Templates.count_templates(), 2)
    ...
    def testDatabase(self):
        self.assertEqual(Database.get_records(), [])
        ...
        item = Database.fetch_item(itemId)

        self.assertIsInstance(item, Database)
        self.assertEqual(Database.get_records(), [item])
        self.assertEqual(Database.search_records("never"), [item])
        self.assertEqual(
            Database.item_to_dict(item),
            {
                "id": "1",
                "filepath": os.path.join(self.app.config["DOWNLOADS"], "/file.mp3"),
                "name": "Never Gonna Give You Up",
        ...
        self.assertIs(Database.check_file(item.filepath), item)  # type: ignore
        self.assertIs(Database.check_yt(item.youtube_id), item)  # type: ignore
        self.assertIs(Database.check_trackid(item.audio_id), item)  # type: ignore
