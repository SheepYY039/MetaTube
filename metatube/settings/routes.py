...
def deltemplate(id):
    ...
    template.delete()
    sockets.templatesettings(
        {"status": "delete", "msg": "Template successfully removed", "templateid": id}
    )
...
def updatesettings(
    ffmpeg_path, amount, hardware_transcoding, metadata_sources, extradata
):
    db_config = Config.query.get(1)
        ...
        if os.path.exists(ffmpeg_path):
            ...
            if db_config.ffmpeg_directory != ffmpeg_path:
                ...
                if ffmpeg_instance.test():
                    response += (
                        "FFmpeg path has succefully been updated and found! <br />"
                    )
                else:
    ...
    if "spotify" in metadata_sources and "spotifyapi" in extradata:
        spotifydata = (
            extradata["spotifyapi"]["secret"] + ";" + extradata["spotifyapi"]["id"]
        )
