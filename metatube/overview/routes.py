...
def index():
    ...
    metadataform = render_template(
        "metadataform.html", metadata_sources=metadata_sources
    )
        ...
        if yt.is_supported(query):
            ...
            if Database.checkyt(video["id"]) is None:
                ...
                socketio.start_background_task(
                    yt.fetch_video, video, templates, metadata_sources, defaulttemplate
                )
            else:
...
def mergedata(metadata, filepath):
    ...
    if (
        Database.checktrackid(release_id) is None
        and Database.checktrackid(metadata.get("trackid", "")) is None
    ):
        ...
        if extension in env.META_EXTENSIONS:
            if source == "Spotify":
                ...
                data = MetaData.getmusicbrainzdata(
...
def downloaditems(items):
    try:
        ...
        try:
            ...
            logger.error(
                "An error occured whilst deleting the temporary file: %s", str(e)
            )
            ...
            sockets.overview(
                {
                    "msg": "play_file",
                    "data": content,
                    "itemdata": Database.itemtodict(item),
                    "mimetype": mimetype,
                }
            )
        else:
